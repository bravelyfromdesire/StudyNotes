...
如果没有找到目标类的文件，则include会抛出一个E_WARNING。

### 2.作用域的继承
被引用文件一般包含了类、变量还有函数。目标文件被引用之后，除了魔法变量(__FILE__等)，其余变量会继承include函数的变量作用域。不过，类还有函数以及类变量是全局的。

### 3.返回值
如果目标文件不存在，则include返回FALSE。
如果引用目标文件成功，返回值包括以下几种情况：
* 默认情况下，引用以及执行目标文件后返回1；
* 目标文件中提前return，则目标文件中后续变量将不可引用，但是函数及类可以引用；

### 4.include是一个语言结构
注意，include是一个语言结构，所以在调用时不需要使用圆括号。这里会有一个陷阱：
